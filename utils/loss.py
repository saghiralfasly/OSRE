...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        if g > 0:
            BCEcls, BCEobj = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g) #*************
            # BCEcls, BCEobj, MSEangle = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g), FocalLoss(BCEangle, g) #*************

        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index   ***********************************************
        self.BCEcls, self.BCEobj, self.MSEangle, self.gr, self.hyp, self.autobalance = BCEcls, BCEobj, MSEangle, model.gr, h, autobalance
        for k in 'na', 'nc', 'nl', 'anchors', 'stride':
            setattr(self, k, getattr(det, k))

    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        # print("input  loss ------------------")   
        # print(type(p), type(targets), type(imgs))
        # print(len(p), p[0].shape, targets.shape, imgs.shape)
        # print("data--")
        # print(p[0])              torch.Size([1, 3, 80, 80, 8])    torch.Size([1, 3, 40, 40, 8])    torch.Size([1, 3, 20, 20, 8])
        # print(targets)          torch.Size([8, 6])    [0.11373, 0.12157, 0.12157,  ..., 0.71765, 0.72941, 0.73725]
        # print(imgs)
        # 
        device = targets.device

        targetsWithoutAngles = targets[:,:6]
        pWithoutAngles = []
        for pp1 in p:
            pWithoutAngles.append(pp1[..., :8])

        # targetsOld = targets
        # print(targets)
        targetDict = {}
        for tr in targets:
            targetDict[str(tr[:6].to(device))] = tr[6:].to(device)

        
        lcls, lbox, lobj, lyz = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targetsNew, anchors = self.build_targets(pWithoutAngles, targetsWithoutAngles, imgs)

        # get angles along with other built target by matching them
        targets = []
        for tr in targetsNew:
            if tr.shape[0] > 0:
                trg = torch.zeros((tr.shape[0],8), device=device)
                trg[:,:6] = tr
                for indx, lbl in enumerate(tr):
                    trg[indx,6:] = targetDict[str(lbl)][:]
            else:
                trg = []
            targets.append(trg)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # angles' regression
                pAnglesyz = ps[:, -2:].sigmoid()# * 2. - 0.5
                tAnglesyz = targets[i][:, -2:]
                lyz += self.MSEangle(pAnglesyz,tAnglesyz)  # BCE
                # pAnglesyz = ps[:, 6:].sigmoid() * 2. - 0.5
        ...
        loss = lbox + lobj + lcls + lyz  # ****************************************
        return loss * bs, torch.cat((lbox, lobj, lcls, lyz, loss)).detach()
