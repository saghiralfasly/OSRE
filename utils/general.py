...
def non_max_suppressionDetect(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=()):
    """Runs Non-Maximum Suppression (NMS) on inference results
    ...
    # for xi, (x, xAll) in enumerate(zip(prediction[...,:-2],prediction)):
    for xi, (x, xAll) in enumerate(zip(prediction,prediction)):  # image index, image inference
        x = x[xc[xi]]  # confidence
        # Cat apriori labels if autolabelling
        if labels and len(labels[xi]):
            l = labels[xi]
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), x[:,-2:], 1) 
        else:  # best class only
